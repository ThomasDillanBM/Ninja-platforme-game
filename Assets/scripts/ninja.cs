...
public class ninja : MonoBehaviour
{
    ...
    private bool jump_flo;
    private float jump_flo_total = 0.2f, jump_flo_elapsed;
	...
	void Update ()
    {
        ...
        if (jump_flo)
        {
            jump_flo_elapsed += Time.deltaTime;
            if(jump_flo_elapsed >= jump_flo_total)
            {
    ...
    void jump()
    {
        ...
        {   //Make the jump
            ...
            //GetComponent<Rigidbody2D>().Sleep();
            //GetComponent<Rigidbody2D>().WakeUp();
            if (chao_state == 1) GetComponent<Rigidbody2D>().AddForce(Vector2.up * jumpForce);
            if (chao_state == 2)
            {
                GetComponent<Rigidbody2D>().AddForce((Vector2.up + Vector2.left) * jumpForce);
                jump_flo = true;
                jump_flo_elapsed = 0;
            }
            if (chao_state == 3)
            {
                GetComponent<Rigidbody2D>().AddForce((Vector2.up + Vector2.right) * jumpForce);
                jump_flo = true;
                jump_flo_elapsed = 0;
            }
